...
function saveToLS(data) { if (typeof window !== 'undefined') localStorage.setItem(LS_KEY, JSON.stringify(data)); }
function loadFromLS() {
...
// -------------- Scoring --------------
function riskScoreFromMchat(template, respostas) {
...
export default function Page() {
  const [store, setStore] = React.useState(loadFromLS());
  ...
  const templatesById = React.useMemo(() => Object.fromEntries(templates.map(t => [t.id, t])), [templates]);
  ...
  function submitResponse(code, data) {
          ...
          <div className="space-y-6">
            ...
            <div className="card">
              ...
              <div className="card-content space-y-3">
                ...
                {Object.values(assignments).sort((a,b)=> (b.createdAt||"").localeCompare(a.createdAt||""))
                  .map((a)=> (
            ...
            <div className="card">
              ...
              <div className="card-content"><QuickTemplateForm onCreate={(t)=> setTemplates(prev => [...prev, t])} domains={domains} /></div>
            </div>
          </div>
        )}
...
function Responder({ code, assignment, templatesById, onSubmit }) {
  ...
  const [form, setForm] = React.useState(Object.fromEntries(template.itens.map(i => [i.id, ""])));
  const [sent, setSent] = React.useState(false);

  function setValue(id, v) { setForm(s => ({ ...s, [id]: v })); }
...
function MchatResultado({ template, respostas }) {
...
// Export JSON
function downloadJSON(assignment) {
...
// Quick Template
function QuickTemplateForm({ onCreate, domains }) {
